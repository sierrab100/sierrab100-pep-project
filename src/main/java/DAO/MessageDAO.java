...
public class MessageDAO {
    ...
    public Message insertMessage(Message message){
        ...
        try{
            String sql = "INSERT INTO message (message_id, posted_by, message_text, time_posted_epoch) values (default, ?, ?, ?);";
            ...
            if(key.next()){
                int generated_message_id = (int) key.getInt(1);
        ...
        public List<Message> getAllMessages(){
            ...
            public Message deleteMessagesWithID(Message message){
                Connection connection = ConnectionUtil.getConnection();
                try{
            ...
            try {
                ...
                PreparedStatement preparedStatement = connection.prepareStatement(sql);
                preparedStatement.setLong(1, posted_by);
                ResultSet rs = preparedStatement.executeQuery();
                while(rs.next()){
                    Message message = new Message(rs.getInt("message_id"), rs.getInt("posted_by"), rs.getString("message_text"), rs.getLong("time_posted_epoch"));
                    messages.add(message);
